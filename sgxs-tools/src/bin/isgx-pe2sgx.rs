...
impl<'a> LayoutInfo<'a> {
	pub fn new(pe: &pe::Pe<'a>) -> Result<LayoutInfo<'a>>  {
		...
		for section in pe.get_sections() {
			if section.name.as_os_str()=="sgxmeta" {
				if let Some(_)=opt_sgxmeta {
					...
					bail!("sgxmeta section is {} bytes, expected at least {} bytes", {section.size_of_raw_data}, sgxmeta_minsize);
				}
				...
				if let Some(_)=opt_tls {
					...
					bail!(".tls section is {} bytes, expected at least {} bytes", {section.size_of_raw_data}, tls_minsize);
				}
				opt_tls=Some(section);
				layout.push(TlsPeSection{header:section,data:pe.ref_slice_at(section.virtual_address,section.size_of_raw_data).map_err(pe_err)?});
			} else {
				layout.push(RegularPeSection{header:section,data:pe.ref_slice_at(section.virtual_address,section.size_of_raw_data).map_err(pe_err)?});
			}
		}
		...
		let sgxmeta=pe.ref_slice_at(sgxmeta_s.virtual_address,sgxmeta_s.size_of_raw_data).map_err(pe_err)?.as_ptr();
		...
		let enclave_entry=match pe.get_exports().map_err(pe_err)?.lookup_symbol("enclave_entry") {
			Ok(pe::ExportAddress::Export(rva)) => { rva.get() as u64 },
			Err(pe::Error::DirectoryMissing) | Err(pe::Error::SymbolNotFound) |
			Err(pe::Error::ExportNotFound) | Ok(_) => { bail!("enclave_entry symbol not found") },
...
impl<'data> PeHeader<'data> {
	fn new(pe: &pe::Pe<'data>) -> Result<PeHeader<'data>> {
		...
		Ok(PeHeader{
			...
			certdir:pe.get_directory_raw(DirectoryEntry::CertificateTable).map_err(pe_err)?,
		})
	}
