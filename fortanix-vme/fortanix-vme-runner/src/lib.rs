...
impl<I> From<(io::Error, I)> for RunnerError  where I : Into<Cow<'static, str>> {
    fn from((e, ctx): (io::Error, I)) -> Self where I : Into<Cow<'static, str>> {
...
impl<P: Platform + 'static> EnclaveRunner<P> {
    ...
    pub fn run_enclave<I: Into<P::RunArgs>>(&mut self, run_args: I, enclave_name: String, enclave_args: Vec<String>) -> Result<(), RunnerError> {
        let server = Arc::new(Server::bind(enclave_name, SERVER_PORT).map_err::<RunnerError,_>(|e| 
