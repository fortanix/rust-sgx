...
impl StreamConnection for VsockStream {
    ...
    fn local(&self) -> io::Result<String> {
        ...
        if let SockAddr::Vsock(addr) = addr.into() {
            Ok(format!("{}", addr.cid()))
        } else {
...
impl Server {
    ...
    fn read_request(stream: &mut VsockStream) -> Result<Request, Error> {
    ...
    fn handle_request_connect(remote_addr: &String, enclave: &mut VsockStream) -> Result<(), IoError> {
        ...
        let response: Vec<u8> = response
                                    .try_into()
                                    .map_err(|_| IoError::new(IoErrorKind::Other, "Serialization failed"))?;
