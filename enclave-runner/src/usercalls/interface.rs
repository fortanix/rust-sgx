...
use super::abi::{Usercalls, UsercallResult};
use super::{RunningTcs, EnclaveAbort};
...
impl<'a> Usercalls for Handler<'a> {
    ...
    fn bind_stream(&mut self, addr: *const u8, len: usize, local_addr: *mut ByteBuffer) -> UsercallResult<(Result, Fd)> {
		unsafe {
    ...
    fn accept_stream(&mut self, fd: Fd, local_addr: *mut ByteBuffer, peer_addr: *mut ByteBuffer) -> UsercallResult<(Result, Fd)> {
		unsafe {
			let mut local_addr = local_addr.as_mut().map(OutputBuffer::new);
			let mut peer_addr = peer_addr.as_mut().map(OutputBuffer::new);
			Ok(self.0.accept_stream(fd, local_addr.as_mut(), peer_addr.as_mut()).to_sgx_result())
		}
    }

    fn connect_stream(&mut self, addr: *const u8, len: usize, local_addr: *mut ByteBuffer, peer_addr: *mut ByteBuffer) -> UsercallResult<(Result, Fd)> {
		unsafe {
			let mut local_addr = local_addr.as_mut().map(OutputBuffer::new);
			let mut peer_addr = peer_addr.as_mut().map(OutputBuffer::new);
			Ok(from_raw_parts_nonnull(addr, len)
				.and_then(|addr| self.0.connect_stream(addr, local_addr.as_mut(), peer_addr.as_mut()))
				.to_sgx_result())
		}
    }
    ...
    fn async_queues(&mut self, usercall_queue: *mut FifoDescriptor<Usercall>, return_queue: *mut FifoDescriptor<Return>) -> UsercallResult<Result> {
		unsafe {
			Ok((|| {
				let usercall_queue = usercall_queue.as_mut().ok_or(IoError::from(IoErrorKind::InvalidInput))?;
				let return_queue = return_queue.as_mut().ok_or(IoError::from(IoErrorKind::InvalidInput))?;
