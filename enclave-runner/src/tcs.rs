...
pub(crate) fn enter<T: Tcs, F>(tcs: T, mut on_usercall: F, p1: u64, p2: u64, p3: u64, p4: u64, p5: u64, debug_buf: Option<&RefCell<DebugBuffer>>) -> (T, DispatchResult)
	where F: FnMut(u64, u64, u64, u64, u64) -> DispatchResult
{
...
impl<T: Tcs> Usercall<T> {
	...
	pub fn coreturn(self, retval: (u64, u64), debug_buf: Option<&RefCell<DebugBuffer>>) -> ThreadResult<T> {
		coenter(self.tcs, 0, retval.0, retval.1, 0, 0, debug_buf)
	}
}

pub(crate) fn coenter<T: Tcs>(tcs: T, mut p1: u64, mut p2: u64, mut p3: u64, mut p4: u64, mut p5: u64, debug_buf: Option<&RefCell<DebugBuffer>>) -> ThreadResult<T> {
	let sgx_result: u32;
