...
use sgx_isa::{Sigstruct, Attributes, AttributesFlags, Miscselect};
use sgxs::loader::{Tcs, Load};
use sgxs::sigstruct::{self, Signer, EnclaveHash};
...
impl EnclaveBuilder {
	...
	fn generate_dummy_signature(&self) -> Result<Sigstruct, Error> {
		...
		signer.attributes_flags(attributes.flags, !0)
	...
	fn load<T: Load>(self, loader: &mut T) -> Result<Vec<ErasedTcs>, Error> {
		...
		let signature = match self.signature {
			Some(sig) => sig,
			None => self.generate_dummy_signature().context("While generating dummy signature")?
		};
		...
		let mapping = loader.load(&mut OpenAndRead::Unopened(&self.path), &signature, attributes, miscselect)?;
