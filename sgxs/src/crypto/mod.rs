...
mod openssl {
	...
	use openssl::pkey::{HasPublic, Public, Private};
	...
	impl SgxHashOps for Hasher {
		...
		fn finish(mut self) -> Hash {
			let mut hash = [0u8; 32]; 
			hash.copy_from_slice(&Hasher::finish(&mut self).expect("failed to finish openssl hasher"));
			hash
		}
	}
	...
	impl<T: HasPublic + MaybePrivate> SgxRsaOps for RsaRef<T> {
		...
		fn verify_sha256_pkcs1v1_5<S: AsRef<[u8]>, H: AsRef<[u8]>>(&self, sig: S, hash: H) -> Result<(),Self::Error> {
			...
			let ret = unsafe {
				...
				// only unhashed data.
				ffi::RSA_verify(Nid::SHA256.as_raw(),
								   hash.as_ref().as_ptr(),
								   hash.as_ref().len() as _,
								   sig.as_mut_ptr(),
								   sig.len() as _,
