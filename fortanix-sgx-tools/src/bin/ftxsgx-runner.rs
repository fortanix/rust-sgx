...
fn main() -> Result<(), Error> {
	...
	let file = match args.next() {
		...
		None => {
			...
			eprintln!("Usage: {} <ENCLAVE>\n", cmdname.unwrap_or("ftxsgx-runner".into())); // extra newline
			bail!("Missing <ENCLAVE> parameter on command line");
		}
	};
	let mut device = IsgxDevice::open(DEFAULT_DEVICE_PATH, AesmClient::new()).context("While opening SGX device")?;
