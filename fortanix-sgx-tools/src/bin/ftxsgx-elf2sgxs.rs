...
fn u64_to_bytes(v: u64) -> Vec<u8> {
	unsafe { std::slice::from_raw_parts(&v as *const u64 as *const u8, std::mem::size_of_val(&v)).to_vec() }
}
...
macro_rules! check_size {
	($syms:ident.$name:ident == $size:expr) => {{
		...
		if size != $size {
			bail!("Dynamic symbol {} doesn't have the right size. Expected size {}, got {}.", stringify!($name), $size, size);
		}
	}}
}
...
impl<'a> LayoutInfo<'a> {
	...
	fn check_symbols(elf: &ElfFile<'a>) -> Result<Symbols<'a>,Error> {
		let dynsym = elf.find_section_by_name(".dynsym").ok_or_else(||format_err!("Could not find dynamic symbol table!"))?;

		let syms = if let SectionData::DynSymbolTable64(syms) = dynsym.get_data(&elf).map_err(err_msg)? {
			syms
		} else {
	...
	fn check_dynamic(elf: &ElfFile<'a>) -> Result<Option<Dynamic<'a>>,Error> {
		...
		let dynh = elf.program_iter().find(|ph| ph.get_type()==Ok(PhType::Dynamic))
			...
			if let SectionData::Rela64(relas) = section.get_data(&elf).map_err(err_msg)? {
				...
				for rela in relas {
					...
					if writable_ranges.iter().find(|r|offset>=r.start && (offset+8)<=r.end).is_none() {
						bail!("Relocation at 0x{:016x} outside of writable segments", offset);
					}
				}
			}
		}
	...
	pub fn new(elf: ElfFile<'a>, ssaframesize: u32, heap_size: u64, stack_size: u64, threads: usize, debug: bool, library: bool) -> Result<LayoutInfo<'a>,Error>  {
		...
		Ok(LayoutInfo{ elf, sym, dyn, ssaframesize, heap_size, stack_size, threads, debug, library })
	}
	...
	pub fn write_elf_segments<W: SgxsWrite>(&self, writer: &mut CanonicalSgxsWriter<W>, heap_addr: u64, memory_size: u64) -> Result<(),Error> {
		let mut splices=[
			...
			Splice(self.sym.RELA.value(),u64_to_bytes(self.dyn.as_ref().and_then(|d|d.rela.get_ptr().ok()).unwrap_or(0))),
			Splice(self.sym.RELACOUNT.value(),u64_to_bytes(self.dyn.as_ref().and_then(|d|d.relacount.get_val().ok()).unwrap_or(0))),
		...
		for ph in self.elf.program_iter().filter(|ph|ph.get_type()==Ok(PhType::Load)) {
			...
			if ph.flags().is_read() { secinfo.flags.insert(SecinfoFlags::R); }
			if ph.flags().is_write() { secinfo.flags.insert(SecinfoFlags::W); }
			if ph.flags().is_execute() { secinfo.flags.insert(SecinfoFlags::X); }
			...
			while cur_splice.peek().map_or(false, |s| s.0>=base && (s.0 + (s.1.len() as u64)) < end ) {
	...
	pub fn write<W: SgxsWrite>(&self, writer: &mut W) -> Result<(),Error> {
		let max_addr = self.elf.program_iter()
			.filter_map( |ph|
			...
			).max().ok_or_else(||format_err!("No loadable segments found"))?;
		...
		for i in 0..self.threads {
			...
			let tls=unsafe{std::mem::transmute::<_,[u8;32]>([stack_tos,secondary as u64,0u64,0u64])};
			let secinfo=SecinfoTruncated{flags:SecinfoFlags::R|SecinfoFlags::W|PageType::Reg.into()};
...
fn check_heap_size(heap_size: u64) {
	if !heap_size.checked_sub(0x1000).map(u64::is_power_of_two).unwrap_or(false) {
		println!("WARNING: heap size is not a power of two plus one page, possible waste of memory");
	}
}
...
fn main_result(args: ArgMatches) -> Result<(),Error> {
	...
	let layout = LayoutInfo::new(srcelf,ssaframesize,heap_size,stack_size,threads,debug,library)?;

	let mut outfile=args.value_of("output").map(|out|File::create(out))
...
fn main() {
	...
	let args = App::new("libenclave-elf2sgxs")
		...
		.arg(Arg::with_name("debug")     .short("d").long("debug").help("Set enclave debug mode"))
		.arg(Arg::with_name("library")              .long("library").help("This is a library enclave (experimental)"))
		.arg(Arg::with_name("output").short("o").long("output").value_name("FILE").help("Specify output file"))
		.arg(Arg::with_name("elf").index(1).required(true).help("Path to the ELF binary to be converted"))
		.get_matches();
