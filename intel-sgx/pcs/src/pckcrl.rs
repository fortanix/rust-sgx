...
impl PckCrl<Unverified> {
    ...
    pub fn verify<B: Deref<Target = [u8]>>(
        ...
        let mut pck_ca = Certificate::from_pem(pck_ca.as_bytes_with_nul())
            .map_err(|_| Error::InvalidCaFormat)?;
...
impl<V: VerificationType> PckCrl<V> {
    pub(crate) fn filename_from_ca(ca: DcapArtifactIssuer) -> String {
        match ca {
    ...
    pub fn ca(&self) -> Result<DcapArtifactIssuer, Error> {
        let issuer = crate::as_mbedtls_crl(&self.crl)
...
mod tests {
    ...
    use {super::PckCrl, crate::DcapArtifactIssuer};

    #[cfg(all(not(target_env = "sgx"), feature = "verify"))]
    #[test]
    fn read_pck_crl() {
        let crl =
            PckCrl::read_from_file("./tests/data/", DcapArtifactIssuer::PCKProcessorCA).unwrap();
    ...
    fn serialize_deserialize_pck_crl() {
        ...
        let crt_deserialized: PckCrl<crate::Unverified> =
            serde_json::de::from_slice(&serialized_bytes[..]).unwrap();
    ...
    fn read_platform_pck_crl() {
        let pckcrl =
            PckCrl::read_from_file("./tests/data/", DcapArtifactIssuer::PCKPlatformCA).unwrap();
        let root_ca = include_bytes!("../tests/data/root_SGX_CA_der.cert");
        let root_cas = [&root_ca[..]];
