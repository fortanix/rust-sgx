...
use sgxs_crate::sgxs::{SgxsRead,PageReader,CreateInfo,MeasECreate,MeasEAdd,PageChunks,Error as SgxsError};
use sgxs_crate::loader;
use sgxs_crate::einittoken::EinittokenProvider;

use abi::{Sigstruct,Einittoken,Secs,Secinfo,PageType,ErrorCode,Miscselect,Attributes};
...
impl Mapping {
	fn new(dev: Arc<InnerDevice>, size: u64) -> Result<Mapping> {
		let ptr=unsafe{libc::mmap(ptr::null_mut(),size as usize,libc::PROT_READ|libc::PROT_WRITE|libc::PROT_EXEC,libc::MAP_SHARED,dev.fd.as_raw_fd(),0)};
	...
	fn create(&mut self, ecreate: MeasECreate, attributes: Attributes, miscselect: Miscselect) -> Result<()> {
		...
		try_ioctl_unsafe!(Create,ioctl::create(self.device.fd.as_raw_fd(),&createdata));
		Ok(())
	}
...
impl<P: EinittokenProvider> loader::Load for Device<P> {
	...
	fn load<R: SgxsRead>(&mut self, reader: &mut R, sigstruct: &Sigstruct, attributes: Attributes, miscselect: Miscselect) -> ::std::result::Result<loader::Mapping<Self>, ::failure::Error> {
		let einittoken = self.einittoken_provider.token(sigstruct, attributes, false).context("The EINITTOKEN provider didn't provide a token")?;
		...
		match mapping.init(sigstruct, &einittoken) {
			Err(ref e) if e.is_einittoken_error() => {
				let einittoken = self.einittoken_provider.token(sigstruct, attributes, true).context("The EINITTOKEN provider didn't provide a token")?;
		...
		Ok(loader::Mapping {
			tcss: mapping.tcss.iter()
				.map(|&tcs| Tcs { _mapping: mapping.clone(), address: tcs })
				.collect(),
